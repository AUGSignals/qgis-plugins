...
import os.path
# Import the code for the dialog
from .image_fusion_dialog import ImageFusionDialog
import os.path
...
class ImageFusion:
    ...
    def addToCustomMenu(self):
        ...
        self.action = QAction(QIcon(":/plugins/image_fusion/icon.png"),
                                    "Image Fusion",
                                    self.iface.mainWindow())
        self.action.setObjectName("Image Fusion")
        self.action.setWhatsThis("Configuration for test plugin")
        self.action.setStatusTip("Image Fusion")
        self.action.triggered.connect(self.run)

        # Create list containing submenu actions from main menu
        submenus = []
        for item in self.menu.actions():
            if item.text() == '&Image Fusion':
        ...
        if not submenus:
            self.subMenu = self.menu.addMenu( '&Image Fusion')

        self.subMenu.setObjectName("&Image Fusion")
    ...
    def run(self):
        ...
        if result:
            self.arguments['-s'] = self.dlg.sarQgsFileWidget.filePath()
            self.arguments['-p'] = self.dlg.opticalQgsFileWidget.filePath()
            self.arguments["-o"] = self.dlg.outputQgsFileWidget.filePath()
            self.arguments["-r"] = str(self.dlg.redLevelQgsDoubleSpinBox.text())
            self.arguments["-g"] = str(self.dlg.greenLevelQgsDoubleSpinBox.text())
            self.arguments["-b"] = str(self.dlg.blueLevelQgsDoubleSpinBox.text())
            self.arguments["-l"] = self.dlg.singleBandCheckBox.isChecked()
